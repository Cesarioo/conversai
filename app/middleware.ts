...
export async function middleware(req: NextRequest) {
  ...
  const publicRoutes = ['/login', '/setup-account', '/auth/callback']
  const isPublicRoute = publicRoutes.some(route => req.nextUrl.pathname.startsWith(route))
  
  // Allow Supabase auth verification URLs to pass through
  if (req.nextUrl.pathname.includes('/auth/v1/verify')) {
    return res
  }
